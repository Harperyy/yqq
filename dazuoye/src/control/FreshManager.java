...
public class FreshManager {
    ...
    public void UpFre(int id,String name,double price,double vip_p,int count,double size,String remark)throws BaseException{
        Connection conn = null;
        ...
        try{
            ...
            rs.close();
            pst.close();
            sql = "update  fresh set fre_name=?,Fre_price=?,Fre_vip_price=?,Fre_count=?,Fre_size=?,fre_remark=?where fre_id=?";
            pst = conn.prepareStatement(sql);
            pst.setString(1,name);
